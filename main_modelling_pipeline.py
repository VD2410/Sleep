...
class Modelling:
    ...
    def sleep_minutes_model(self, sleep_minutes_df, target_feature, user_id, shift_instance):
        ...
        print(f'Debug: X_train.shape before forcing reshape: {X_train.shape}')
        if len(X_train.shape) == 1:
            X_train = X_train.reshape(-1, 1)  # Convert to 2D array if 1D
        print(f'Debug: X_train.shape after possible reshape: {X_train.shape}')
        input_shape = (X_train.shape[1],)
        print(f'Debug: Final input_shape: {input_shape}')
        print(f'Debug: X_train.shape before forcing reshape: {X_train.shape}')
        if len(X_train.shape) == 1:
            X_train = X_train.reshape(-1, 1)  # Convert to 2D array if 1D
        print(f'Debug: X_train.shape after possible reshape: {X_train.shape}')
        ...
        print(f'Debug: Final input_shape: {input_shape}')
        input_shape = (X_train.shape[1],)
        print(f'Debug: Defined input_shape as: {input_shape}')
